    ...
    export const addVariant = async (req, res) => {
        try {
            const { productReference } = req.params; // Referencia del producto
            const variantData = req.body; // Datos de la nueva variante
    
            if (!variantData || Object.keys(variantData).length === 0) {
                return res.status(400).json({ message: "Faltan datos de la variante." });
            }
            ...
            // Generar un código único para la nueva variante
            let productCode;
            let isUnique = false;
            ...
            // Crear un nuevo ID para la variante
            const variantId = new mongoose.Types.ObjectId();
            ...
            const newVariant = {
                ...
                showing_image: variantData.showing_image || "",
            };
    
            // Agregar la nueva variante al producto
